            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Check out reports for {{$organisation->name}}
                        </h3>
                                        ...
                                        <div class="input-group">
                                            ...
                                            <div class="cs-hidden">
                                                <input type="text" name="report_id" class="form-control mr-2"
                                                       value="{{\Illuminate\Support\Facades\Crypt::encrypt(1005)}}">
                                            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table data-list-view dataex-html5-selectors">
                                ...
                                @php
                                    $count=1;
                                @endphp
                                @foreach($organisation_visitors as $visit)
                                    @if($visit->visitor_state!=3 || $visit->organisation_id !=$organisation->id)
                                        @continue
                                    @endif
                                    <tr>
                                        <td class="small" title="{{$visit->id}}"></td>
                                        <td class="small" class="small">{{$count++}}</td>
                                        <td class="small">{{$visit->first_name." ".$visit->last_name}}</td>
                                        <td class="small text-lowercase">{{$visit->email}}</td>
                                        <td class="small">{{$visit->office}}</td>
                                        <td class="small">{{$visit->phone}}</td>
                                        <td class="small">{{$visit->national_id}}</td>
                                        <td class="small">{{date_format(date_create($visit->date_of_visit), 'd/m/Y')}}</td>
                                        <td class="small">{{date_format(date_create($visit->time_in), 'g:i A')}}</td>
                                        <td class="small">{{date_format(date_create($visit->time_out), 'g:i A')}}</td>
                                        <td class="small">
